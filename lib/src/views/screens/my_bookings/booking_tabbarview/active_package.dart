...
class ActivePackageTabBarView extends StatefulWidget {
  const ActivePackageTabBarView({Key? key}) : super(key: key);

  @override
  State<ActivePackageTabBarView> createState() => _ActivePackageTabBarViewState();
}

class _ActivePackageTabBarViewState extends State<ActivePackageTabBarView> {
  ...
  getdata()async{
    var a = await tourController.tourBookings(value: '1');
    if(a == true){
      loading.value = false;
      // loading.refresh();
    }
  ...
  Widget build(BuildContext context) {
    return Obx(
      ()=> Stack(
        children: [
          if(tourController.myTours.isEmpty || (tourController.myTours.length == 1 && tourController.myTours[0].id == 0))Center(child: Text('No Active Tours.'),)
