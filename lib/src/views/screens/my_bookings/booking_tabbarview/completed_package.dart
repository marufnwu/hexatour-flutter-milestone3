...
class CompletedPackageTabBarView extends StatefulWidget {
  const CompletedPackageTabBarView({Key? key}) : super(key: key);

  @override
  State<CompletedPackageTabBarView> createState() => _CompletedPackageTabBarViewState();
}

class _CompletedPackageTabBarViewState extends State<CompletedPackageTabBarView> {
  ...
  getdata()async{
    var a = await tourController.tourBookings(value: '2');
    if(a == true){
      loading.value = false;
      // loading.refresh();
    }
  ...
  Widget build(BuildContext context) {
    return Obx(
          ()=> Stack(
        children: [
          if(tourController.myTours.isEmpty || (tourController.myTours.length == 1 && tourController.myTours[0].id == 0))Center(child: Text('No Completed Tours.'),)
